...
- **Protocol parsing** — Ethernet → IP → TCP/UDP → HTTP/DNS parsing chain using struct overlays on raw memory
- **Connection tracking** — 5-tuple flow table (src IP, dst IP, src port, dst port, protocol) with TCP state machine (SYN_SENT → ESTABLISHED → CLOSING → RESET)
...
ETH  a4:c3:f0:12:3b:cc → 00:15:5d:64:f4:c4  type=0x0800
IP   172.31.185.120  → 140.82.112.21    proto=6  ttl=64  len=60
TCP  172.31.185.120:43614 → 140.82.112.21:443    [SYN]  seq=340906983

Packet length: 74 bytes
ETH  00:15:5d:64:f4:c4 → a4:c3:f0:12:3b:cc  type=0x0800
IP   140.82.112.21   → 172.31.185.120   proto=6  ttl=53  len=60
TCP  140.82.112.21:443   → 172.31.185.120:43614  [SYN ACK]  seq=376952304
...
ETH  00:15:5d:64:f4:c4 → a4:c3:f0:12:3b:cc  type=0x0800
IP   8.8.8.8         → 172.31.185.120   proto=17  ttl=123  len=83
UDP  8.8.8.8 → 172.31.185.120
DNS  REPLY  A     google.com → 142.250.190.238  ttl=300s

Packet length: 224 bytes
ETH  a4:c3:f0:12:3b:cc → 00:15:5d:64:f4:c4  type=0x0800
IP   172.31.185.120  → 172.66.152.176   proto=6  ttl=64  len=210
TCP  172.31.185.120:56162 → 172.66.152.176:80    [ACK PSH]  seq=2821527966
     HTTP: GET /ubuntu/pool/main/l/lmdb/liblmdb0_0.9.31-1_amd64.deb HTTP/1.1
...
Top hosts:
  172.31.185.120     85 packets
  140.82.112.21      32 packets
  8.8.8.8            12 packets
  172.66.152.176     8 packets
  185.125.190.57     4 packets

=== Connection Table ===
Source                 Destination            Proto  Packets Bytes
---------------------------------------------------------------------------
172.31.185.120:43614   140.82.112.21:443      TCP    32      48 KB   ESTABLISHED
172.31.185.120:56162   172.66.152.176:80      TCP    8       312 KB  CLOSING
172.31.185.120:34622   8.8.8.8:53             UDP    6       480 B   UDP
========================
======================
```
...
└──────────────────────┬──────────────────────────────────┘
                       │ thread-safe queue (mutex + condvar)
┌──────────────────────▼──────────────────────────────────┐
│                    Parser Thread                         │
...
**Parsing chain:** Each layer casts the raw byte buffer to a packed struct using `reinterpret_cast`, reads the relevant fields with network-to-host byte order conversion (`ntohs`/`ntohl`), then advances the pointer past the header and calls the next layer's parser.
...
packet-sniffer/
├── src/
│   ├── main.cpp           # Entry point, signal handling, session summary
│   ├── cli.h / cli.cpp    # Argument parsing (getopt)
│   ├── capture.h / capture.cpp      # libpcap interface, thread management
│   ├── packet_queue.h / packet_queue.cpp  # Thread-safe producer-consumer queue
│   ├── ethernet.h / ethernet.cpp    # Layer 2 parsing
│   ├── ip.h / ip.cpp                # Layer 3 parsing
│   ├── tcp.h / tcp.cpp              # Layer 4 TCP + HTTP
│   ├── dns.h / dns.cpp              # DNS query/response reconstruction
│   ├── connection.h / connection.cpp # 5-tuple flow table + TCP state machine
│   └── stats.h                      # Session statistics
└── Makefile
```
...
- HTTPS traffic (port 443) is encrypted — TCP connections are tracked but payload content is not readable
